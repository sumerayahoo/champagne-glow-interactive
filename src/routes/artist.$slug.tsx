...
function ArtistPage() {
  ...
  return (
    <div className="relative min-h-screen text-foreground" dir={isAr ? "rtl" : "ltr"}>
      <Petals />
      <header className="relative z-10 mx-auto max-w-6xl px-6 py-8 flex items-center justify-between gap-3 flex-wrap">
        <button
          type="button"
          onClick={() => router.history.back()}
          className="inline-flex items-center gap-2 rounded-full border border-primary/40 bg-card/40 px-4 py-2 text-sm backdrop-blur-md hover:bg-primary hover:text-primary-foreground transition-all"
        >
          {isAr ? "→ رجوع" : "← Go back"}
        </button>
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="text-sm text-muted-foreground hover:text-primary">{t("Atelier home", "الرئيسية")}</Link>
        </div>
      </header>
